...
(function songstats() {
  ...
  async function makePlaylist(uris) {
    ...
    const playlistitems = (
      await CosmosAsync.get('https://api.spotify.com/v1/playlists/' + uriFinal + '/tracks')
    ).items.map((i) => i.track.href);
    ...
    for (var i = 0; i < 21; i++) {
      ...
      function validateSong(res2) {
        if (
          Math.round(100 * res2.liveness) / 100 >= avr2Liveness - 2 &&
          Math.round(100 * res2.liveness) / 100 <= avr2Liveness + 2
        ) {
        } else {
          return false;
        ...
        if (
          Math.round(100 * res2.energy) / 100 >= avr2Energy - 2 &&
          Math.round(100 * res2.energy) / 100 <= avr2Energy + 2
        ) {
        } else {
          return false;
        ...
        if (
          Math.round(100 * res2.danceability) / 100 >= avr2Dance - 2 &&
          Math.round(100 * res2.danceability) / 100 <= avr2Dance + 2
        ) {
        } else {
          return false;
      ...
      if (randomSongrequestToAppend[0] != undefined) {
        try {
          res2 = await CosmosAsync.get(
            'https://api.spotify.com/v1/audio-features/' + randomSongrequestToAppend[0].split(':')[2],
          );
